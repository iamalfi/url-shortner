...
const redisClient = redis.createClient(
    19726, // port-number
    "redis-19726.c90.us-east-1-3.ec2.cloud.redislabs.com", //host name and url of redis
    { no_ready_check: true }
);

redisClient.auth("18dGRhBYfQKVpGARvLaYLwJ0VLHKUUkr", function (err) {
    // it authenticate the client
    if (err) throw err;
});
    ...
    try {
        ...
        if (Object.keys(data).length == 0)
            return res.status(400).send({
                status: false,
                message: "please enter some data in body",
            });

        if (!isValid(reqLongUrl))
            return res
                .status(400)
                .send({ status: false, message: "please provide the longUrl" });
        reqLongUrl = reqLongUrl.trim();

        if (!validUrl.isWebUri(reqLongUrl))
            return res
                .status(400)
        ...
        urlDetails = await urlModel
            .findOne({ longUrl: reqLongUrl })
...
exports.geturl = async function (req, res) {
    try {
        ...
        let checkUrlCode = await urlModel
